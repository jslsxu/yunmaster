...
public class OpenMapAppUtils {
	...
	public static void startNative_Baidu(Context context, Location loc1, Location loc2){
		...
		try {
			Intent intent = Intent.getIntent("intent://map/direction?origin=latlng:"+loc1.getStringLatLng()+"|name:"+loc1.getAddress()+"&destination=latlng:"+loc2.getStringLatLng()+"|name:"+loc2.getAddress()+"&mode=driving&src=好材#Intent;scheme=bdapp;package=com.baidu.BaiduMap;end");
	...
	public static void startNative_Gaode(Context context, Location loc){
		...
		try {
			...
			android.net.Uri.parse("androidamap://navi?sourceApplication=好材&lat="+loc.getLat()+"&lon="+loc.getLng()+"&dev=1&style=2"));
